...
if __name__ == "__main__":
    ...
    fn.load_state_dict(torch.load("./models/NN/model_C5_10_aprilie_5", weights_only=True))
    fn.eval()
    ...
    for h in range(24):
        if len(traffic[h]) > 0:
            traffic[h] = np.max(traffic[h])
        else:
            traffic[h] = 0

        for d in range(7):
            if len(week_traffic[d][h]) > 0:
                week_traffic[d][h] = np.max(week_traffic[d][h])
            else:
                week_traffic[d][h] = 0
    ...
    with open("pickles/ffnn5.pkl", "wb") as f:
        dump(week_df, f)
